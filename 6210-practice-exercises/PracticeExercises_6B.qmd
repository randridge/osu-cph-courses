---
title: "PUBHBIO 6210: Practice Exercises 6B"
format:
  html:
...
::: {.answer .hidden}
| Group      | Low Birthweight | Not Low Birthweight | Total |
|------------|-----------------|---------------------|-------|
| Smoker     | 30              | 44                  | 74    |
| Non-Smoker | 29              | 86                  | 115   |
| Total      | 59              | 130                 | 189   |
...
::: {.answer .hidden}
Among women who smoke: $\frac{30}{74}=0.405$
...
::: {.answer .hidden}
Common (overall) probability of low birthweight: $\hat{p}=\frac{30+29}{189}=\frac{59}{189}=0.312$

Women who smoke:

Expected \# with low birthweight = $0.312 \times 74 = 23.1$

Expected \# with not low birthweight = $(1-0.312) \times 74 = 50.9$

Women who do not smoke:

Expected \# with low birthweight = $0.312 \times 115 = 35.9$

Expected \# with not low birthweight = $(1-0.312) \times 115 = 79.1$
:::

(d) Calculate the chi-square test statistic using the expected counts you calculated in (c).

::: {.answer .hidden}
$\displaystyle X^2 = \sum_{\text{all cells}} \frac{(\text{Observed} - \text{Expected})^2}{\text{Expected}}$

$\displaystyle = \frac{(30-23.1)^2}{23.1} + \frac{(44-50.9)^2}{50.9} + \frac{(29-35.9)^2}{35.9} + \frac{(86-79.1)^2}{79.1}$
...
::: {.answer .hidden}
$H_0:$ smoking and having a low birthweight baby are not associated
...
p-value = $P(\chi^2(1) > 4.93) = 0.026$ from Stata Stata code: \texttt{display chi2tail(1, 4.93)}
...
A study was done to compare the rate of chronic kidney disease (CKD) among diabetics and non-diabetics. The data from Stata are below. We are interested in whether the probability of CKD is the same for diabetics and non-diabetics.
...
::: {.answer .hidden}
$p_1=$ probability of CKD among diabetics

$p_2=$ probability of CKD among non-diabetics

$\displaystyle \hat{p}_1=\frac{30}{110}=0.273$
...
And also the sample sizes: $n_1=110, n_2=225$

$\displaystyle z=\frac{\hat{p}_1-\hat{p}_2}{\sqrt{\hat{p}(1-\hat{p})\left(\frac{1}{n_1}+\frac{1}{n_2}\right)}} = \frac{0.273-0.156}{\sqrt{0.194(1-0.194)\left(\frac{1}{110}+\frac{1}{225}\right)}}=\frac{0.117}{0.0460} = 2.54$
...
::: {.answer .hidden}
Hypotheses same as in (b), or we can write:

$H_0:$ diabetes and CKD are not associated (are independent)

$H_a:$ diabetes and CKD are associated (are not independent)
...
p-value = $P(\chi^2(1) > 6.49) = 0.011$ from Stata Stata code: \texttt{display chi2tail(1,6.49)}
