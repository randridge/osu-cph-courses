---
title: "PUBHBIO 6211: Practice Exercises Block 7"
format:
  html:
...
We have a sample of 200 people, with age (in years) and bond mineral density (**bmd**) for each person. A simple linear regression model was run using age to predict BMD. Output is below.
...
::: {.answer .hidden}
$\hat{\beta}_{age}=-0.0046$. Estimated mean BMD decreases by 0.0046 units for each 1 year increase in age.
...
![Q-Q Plot](./figures/regress_bmd_age_qnorm_ehat.png){fig-alt="A scatter plot comparing 'Residuals' on the vertical axis against 'Inverse Normal' on the horizontal axis. The x-axis ranges from -0.4 to 0.4, and the y-axis ranges from -0.5 to 1. A solid diagonal reference line passes through the origin. Dark blue data points follow the line closely in the center but deviate at the ends, sitting below the line on the far left and curving significantly above the line on the far right. The highest point is located near (0.4, 0.7) and the lowest near (-0.45, -0.5), and both of these are noticably further from the line than the rest of the points." width=40%}
...
::: {.answer .hidden}
Using a cutoff of $\hat{r}< -3$ or $\hat{r}> 3$ as indicating outliers, IDs 130 and 151 are potential outliers.
...
![Plot without ID labels](./figures/regress_bmd_age_lvr2plot.png){fig-alt="A scatter plot comparing Leverage on the vertical axis against Normalized residual squared on the horizontal axis. The x-axis ranges from 0 to 0.08, and the y-axis ranges from 0 to 0.04. Two red reference lines intersect near the origin, dividing the plot into four quadrants. Most dark blue data points are densely clustered in the bottom-left quadrant. A few points are scattered in the top-left and bottom-right, with one notable outlier located in the far top-right near (0.078, 0.035)." width=40%}

![Plot with ID labels](./figures/regress_bmd_age_lvr2plot_IDs.png){fig-alt="A scatter plot comparing Leverage on the vertical axis against Normalized residual squared on the horizontal axis. The x-axis ranges from 0 to 0.08, and the y-axis ranges from 0 to 0.04. Two red reference lines intersect near the origin, dividing the plot into four quadrants. Most dark blue data points are densely clustered in the bottom-left quadrant. A few points are scattered in the top-left and bottom-right, with one notable outlier located in the far top-right near (0.078, 0.035). The point in the top right has ID = 151. There are also two points with large normalized residual squared values (IDs 130 and 164) but they do not have high leverage as they are close to the red horizontal reference line." width=40%}
...
![Plot without ID labels](./figures/regress_bmd_age_dfbeta.png){fig-alt="A box-and-whisker plot displaying 'Dfbeta age' on the vertical axis, which ranges from -0.2 to 0.8. The main blue box is centered horizontally and sits between -0.05 and 0.05, with a median line near zero. Vertical whiskers extend to approximately 0.15 and -0.15. Several dark blue individual data points are plotted as outliers beyond the whiskers. A few outliers are below the bottom whisker, reaching -0.2, while a larger string of outliers extends upward, with the most extreme point at approximately 0.73." width=40%}

![Plot with ID labels](./figures/regress_bmd_age_dfbeta_IDs.png){fig-alt="A box-and-whisker plot displaying 'Dfbeta age' on the vertical axis, which ranges from -0.2 to 0.8. The main blue box is centered horizontally and sits between -0.05 and 0.05, with a median line near zero. Vertical whiskers extend to approximately 0.15 and -0.15. Several dark blue individual data points are plotted as outliers beyond the whiskers. A few outliers are below the bottom whisker, reaching -0.2, while a larger string of outliers extends upward, with the most extreme point at approximately 0.73 (ID=151)." width=40%}
...
::: {.answer .hidden}
$\hat{\beta}_{age}=-0.0050$. Estimated mean BMD decreases by 0.0050 units for each 1 year increase in age.
...
Based on the CPR plot below, it appears that there may be a non-linear effect of IL-6, and a log-transformation may be appropriate. Thus, The model was run using natural log-transformed IL-6. Stata output is below the figure.

![CPR Plot](./figures/regress_depress_il6_age_cprplot.png){fig-alt="A scatter plot displaying 'Component plus residual' on the vertical axis against 'Test' on the horizontal axis. The x-axis ranges from 0 to 50, and the y-axis ranges from -4 to 6. Dark blue data points are densely clustered between 0 and 15 on the x-axis and become sparse as values increase toward 50. Two trend lines—one straight teal line and one slightly jagged green line—both show a positive slope, rising from near (0, 0) to approximately (50, 4). The spread of points is fairly consistent across the range, with a few outliers appearing above 4 and below -2 on the y-axis." width=40%}
...
22.6% of the variability in depression score is explained by log-transformed IL-6 and age.
...
![Histogram of Residuals](./figures/regress_tnf_sex_age_ehat_hist.png){fig-alt="A density histogram with 'Residuals' on the horizontal axis and 'Density' on the vertical axis. The x-axis ranges from -2 to 2, and the y-axis ranges from 0 to 0.6. Ten tan-colored bars represent the distribution. The bars are tallest near the center, between -0.5 and 0.5, with the highest peak reaching approximately 0.64. The distribution is roughly bell-shaped but exhibits a slight right skew, with shorter bars extending further toward the positive end of the axis than the negative end." width=40%}

![RVF Plot](./figures/regress_tnf_sex_age_rvfplot.png){fig-alt="A scatter plot with Residuals on the vertical axis and Fitted values on the horizontal axis. The x-axis ranges from 1.2 to 2.2, and the y-axis ranges from -1 to 2. The dark blue data points are distributed in a random cloud across the plot, centered around zero on the y-axis. The vertical spread of the points appears relatively constant as the fitted values increase, though the density of points is slightly higher on the right side of the plot." width=40%}
...
::: {.answer .hidden}
$\hat{\beta}_{age}=0.022 \rightarrow 100 \times \left(e^{\hat{\beta}_{age}} - 1 \right) = 100 \times \left(e^{0.022} - 1 \right) = 100 \times (1.022 - 1) = 2.2$
...
::: {.answer .hidden}
$\hat{\beta}_{sex}=0.296 \rightarrow 100 \times \left(e^{\hat{\beta}_{sex}} - 1 \right) = 100 \times \left(e^{0.296} - 1 \right) = 100 \times (1.344 - 1) = 34.4$
