---
title: "PUBHBIO 6211: Practice Exercises Block 1"
format:
  html:
...
43% of the variability in SBP is explained by age.
:::
...
::: {.answer .hidden}
Because we rejected $H_0: \beta_1=0$, we **do not** expect 0 to be in the interval.
...
![Plot A](./figures/regress_mpg_pounds_qnorm_ehat.png){fig-alt="A scatter plot with a horizontal axis labeled 'Inverse Normal' and a vertical axis labeled 'Residuals.' The x-axis ranges from -10 to 10, while the y-axis ranges from -10 to 15. A solid, straight diagonal reference line is plotted. Dark blue data points follow the line closely in the center but form an S-shaped curve, sitting below the line on the lower left and rising significantly above the line on the upper right, ending with a single prominent outlier at approximately (8, 14)." width=40%}

![Plot B](./figures/regress_mpg_pounds_hist_ehat.png){fig-alt="A histogram with horizontal axis labeled 'Residuals' and vertical axis labeled 'Density' that shows a relatively normal distribution with slight right skew." width=40%}

![Plot C](./figures/regress_mpg_pounds_rvfplot.png){fig-alt="A scatterplot with horizontal axis labeled 'Fitted values' and vertical axis labeled 'Residuals'. The points are a mostly random scatter with possibly a slight fan-shape as you move from left to right." width=40%}
