---
title: "PUBHBIO 6211: Practice Exercises Block 3"
format:
  html:
...
A regression model was fit to predict GPA. Use these results to answer the questions.
...
![Plot A](./figures/regress_gpa_age_gender_study_miles_cprplot_study.png){fig-alt="A scatter plot displaying 'Component plus residual' on the vertical axis against 'Avg. hours/week studying' on the horizontal axis. The x-axis ranges from 0 to 50, and the y-axis ranges from -1 to 1.5. Dark blue data points are densely clustered between 0 and 20 hours, often forming vertical columns. Two overlaid trend lines—one straight and one slightly curved—both show a positive slope, rising from near (0, 0) to approximately (50, 0.6). The spread of data points is wider at lower study hours and narrows as they approach 50." width="40%"}

![Plot B](./figures/regress_gpa_age_gender_study_miles_rvfplot.png){fig-alt="A scatter plot comparing 'Residuals' on the vertical axis against 'Fitted values' on the horizontal axis. The x-axis ranges from 2.5 to 4, and the y-axis ranges from -1 to 1. The majority of dark blue data points are densely clustered between fitted values of 2.6 and 3.0, centered around zero. As the fitted values increase toward 4, the points become much more sparse. The distribution appears as a random cloud with no distinct geometric pattern or trend." width="40%"}

![Plot C](./figures/regress_gpa_age_gender_study_miles_qnorm_ehat.png){fig-alt="A scatter plot with 'Residuals' on the vertical axis and 'Inverse Normal' on the horizontal axis. Both axes range approximately from -1 to 1. A solid diagonal reference line runs from the bottom-left to the top-right corner. Numerous dark blue data points are plotted, closely following the diagonal line throughout the entire range. The points exhibit a slight, tight S curve, sitting marginally above the line in the center and marginally below or above at the extreme ends, but generally maintaining a strong linear alignment." width="40%"}
...
::: {.answer .hidden}
$\hat{\beta}_{GPA} = -0.597$

A one point increase in GPA is associated with a 0.597 point decrease in marijuana use score, controlling for popularity score and depression score (p<0.001).
:::
