---
title: "PUBHBIO 6211: Practice Exercises Block 4"
format:
  html:
...
------------------------------------------------------------------------------
   reduction |      Coef.   Std. Err.      t    P>|t|     [95
-------------+----------------------------------------------------------------
...
::: {.answer .hidden}
Yes, meaningful; tests whether there is a non-zero mean reduction in fever for children taking Drug 1.
...
::: {.answer .hidden}
$\hat{E}(REDUCTION | Drug 2) = \hat{\beta}_0 + \hat{\beta}_1 = 1.52 - 1.16 = 0.36$
...
::: {.answer .hidden}
Stay the same: Overall F-test, $R^2$, Adjusted $R^2$\
Change: Intercept estimate, Coefficient for `drug2`
:::
...
A regression model was fit to predict GPA using these variables. Use the Stata output provided below to answer the questions below.
...
::: {.answer .hidden}
$\hat{\beta}_{study}=0.0123, \text{p-value} < 0.0005$. The estimated mean GPA increases by 0.0123 points for each 1 hour studied per week, controlling for religion, age, and gender. This effect is significant (p\$\<\$0.0005).
:::
...
::: {.answer .hidden}
Partial F-test: test statistic $F \sim F$(# of $\beta$s being tested, DF for Residual for Full Model)
...
A regression model was fit to predict drinking score (`drink`) using year in school and sex. Use the Stata output below to answer the questions below.
...
We would like to know whether adding price and foreign-made status improve on a model that includes weight as the only predictor. Use the Stata output below to answer this question. (Perform all steps of the appropriate hypothesis test.) Note that more output is provided than you need for the problem.
...
$F = \frac{\frac{MSS_{Full} - MSS_{Reduced}}{\# \beta\text{s being tested}}}{\text{Mean Square Residual for Full Model}} = \frac{\frac{1620.30716 - 1591.9902}{2}}{11.7593185} = \frac{\frac{28.31696}{2}}{11.7593185} = \frac{14.15848}{11.7593185} = 1.20$
