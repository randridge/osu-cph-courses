...
def clean_latex_formatting(text):
    ...
    # 5. TRANSFORM STATA ENVIRONMENTS TO PROTECTED CODE FENCES
    # Replaces \begin{stata}...\end{stata} with ```{.none .stata} ... ```
    # .none prevents accidental bolding of keywords
    ...
    # Use HTML Entity &#35; globally for \# to prevent any header interpretation
    text = text.replace(r'\#', '&#35;')
    text = text.replace(r'\%', '%')
    text = text.replace(r'\&', '&')
    ...
    # 10. REMOVE SPACING COMMANDS& FIX ALIGNMENT & FIX ALIGNMENT
    text = re.sub(r'\\(quad|qquad|,|:|;)', '', text)
    text = text.expandtabs(4) # Converts tabs to 4 spaces for perfect column alignment
...
def batch_convert():
    ...
    for input_file in tex_files:
        ...
        display_title = f"PUBHBIO 6211: Practice Exercises {suffix}"
