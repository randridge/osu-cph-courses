...
\exercise{9}{A survey of a random sample of students at the University of New Hampshire was conducted. We are interested in predictors of aggression. Students were classified as either having or not having aggression (\texttt{aggression}: 1=has aggression, 0=does not) based on responses to a validated questionnaire. We are interested in potential differences in aggression comparing students who belong and don't belong to a fraternity/sorority (\texttt{frat}: 1=belong to a fraternity or sorority, 0=do not belong to one).}
...
\answer{1.5cm}{$\widehat{OR} = \exp(\hat{\beta}_1) = \exp(0.906812) = 2.48$\\
...
\answer{2.5cm}{Yes, there is evidence that fraternity/sorority membership is associated with aggression, $p=0.013$.}

(c) There is a 95\% CI for \texttt{frat} on the logistic regression output. Report this CI, state what population parameter this is a CI for, and explain how the 95\% CI could be used to answer part (b).

\answer{4cm}{(0.194, 1.62) -- this is a 95\% CI for the true regression coefficient $\beta_1$ (log-odds ratio)\\
...
\answer{2 cm}{$\hat{\beta}_0=0.1636294$\\
\begin{align*}
...
\answer{3cm}{Odds of aggression for people in frat = $\frac{35/47}{12/47} = \frac{35}{12} = 2.9166667$\\
...
\answer{2cm}{$P$(aggression$|$not in frat/sorority$) = \frac{106}{196} = 0.54$\\
Same as answer to (d).}

%%%
\newpage

\exercise{9}{Using the New Hampshire survey, we are also interested in whether age is a predictor of aggression. A logistic regression model was run using age (\texttt{age}, in years) to predict aggression (\texttt{aggression}: 1=has aggression, 0=does not). Note that the \texttt{coef} option was left off the \texttt{logistic} command.}
\begin{stata}
...
------------------------------------------------------------------------------
  aggression | Odds Ratio   Std. Err.      z    P>|z|     [95% Conf. Interval]
-------------+----------------------------------------------------------------
...
\answer{1.5cm}{$\widehat{OR}=0.913$; The estimated odds of aggression multiply by 0.913 for each one year increase in age\\ (The estimated odds decrease by 8.7\% for a one year increase in age).\\
There is not a significant effect of age on aggression, $p=0.068$, though this is close to the $\alpha$ cutoff.}

%%%
\newpage

(b) There is a 95\% CI for \texttt{age} on the logistic regression output. Report this CI, state what population parameter this is a CI for, and explain how the 95\% CI could be used to determine if there is a significant effect of age on aggression.

\answer{1.5cm}{(0.828, 1.007) -- this is a 95\% CI for the true odds ratio, i.e., for $\exp(\beta_1)$\\
...
\exercise{9}{Using the New Hampshire survey, we are also interested in whether there are differences in the probability of having aggression across the years in school. A logistic regression model was run using year in school (\texttt{year}, 1=freshman, 2=sophomore, 3=junior, 4=senior) to predict aggression (\texttt{aggression}: 1=has aggression, 0=does not). Use the Stata output on the next page to answer the questions below.}
...
\answer{2cm}{Yes, p-value = 0.0193 from the ``chunk test" (multivariate Wald test) of the 3 regression coefficients}
...
\answer{5cm}{$\hat{\beta}_{year2}=-0.040822 \rightarrow \widehat{OR}=\exp(\hat{\beta}_{year2}) = \exp(-0.040822) = 0.96$\\
...
\answer{1.5cm}{$\widehat{OR}=\exp(\hat{\beta}_{year3}-\hat{\beta}_{year4})=\exp(0.1823216 - -0.863647)=\exp(1.0459686) = 2.85$
}
%%%
\newpage
