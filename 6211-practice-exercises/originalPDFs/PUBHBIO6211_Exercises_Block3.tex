...
\answer{1.5cm}{$E(MPG) = \beta_0 + \beta_1 WEIGHT + \beta_2 FOREIGN$}
...
\answer{2cm}{The estimated mean change in MPG for a 1 pound increase in weight is -0.0066 miles per gallon, controlling for foreign status. (increase weight, decrease mileage)}

(c) Interpret the value $-1.650029$ from the Stata output.

\answer{2cm}{The estimated mean mileage for foreign cars is 1.65 miles per gallon \underline{lower} than the mean for U.S.-made cars, controlling for weight.}
...
\answer{2cm}{No, test of $H_0: \beta_2=0$ has p-value=0.130}
...
\answer{2cm}{Yes, test of $H_0: \beta_1=0$ has p-value $<$0.0005 (shown on Stata output as 0.000)}

(f) How much of the variability in mileage is explained by foreign status and weight of the car together?

\answer{2cm}{$R^2 = 0.6627 \rightarrow$ 66\% of the variability in mileage is explained by foreign status and car weight.}

%%%
\newpage

\exercise{3}{
A survey of a random sample of students at the University of New Hampshire was conducted. We are interested in predictors of grade point average (GPA), which is measured on a 4-point scale. In particular, we would like to know whether sex is an important predictor (\texttt{gender}; 1=male, 0=female), and also whether how far away a student lives from campus (\texttt{miles}; how many miles away from school the student lives) is an important predictor. We also believe that age (\texttt{age}; years) and how many hours per week a student studies on average (\texttt{study}) might be important predictors.
...
\answer{2cm}{At least one of the predictors -- age, gender, hours studied, and miles away from school -- is significantly associated with GPA (p$<$0.00005).}

(b) The $R^2$ value is missing from the output. Calculate it, and write a one sentence interpretation.

\answer{3cm}{$R^2 = \frac{MSS}{TSS} = \frac{6.670995}{42.3476416} = 0.1575$\\
~\\
...
\answer{2cm}{Adjusted $R^2 = 1-\frac{\text{Mean Square Residual}}{\text{Mean Square Total}} = 1 - \frac{0.179279631}{0.208609072} = 1 - 0.8594 = 0.1406$}

(d) Interpret the coefficient for gender in this model.

\answer{2cm}{$\hat{\beta}_{gender} = -0.124$\\
...
\answer{1.5cm}{$H_0: \beta_{gender}=0$ vs. $H_a: \beta_{gender} \ne 0$\\
...
\answer{2cm}{No; p-value = 0.351}

(h) Interpret the effect of hours studied on GPA, including reference to a p-value.

\answer{2cm}{The estimated mean GPA increases by 0.038 points for each one year increase in age, adjusting for age, gender, and how far away from school a student lives; this is a significant association (p$<$0.0005).}
...
\answer{2cm}{$\widehat{GPA} = 1.87 + 0.038 \times AGE - 0.124 \times GENDER + 0.0128 \times STUDY + 0.00513 \times MILES$\\
$\qquad \quad = 1.87 + 0.038 \times 20 - 0.124 \times 1 + 0.0128 \times 10 + 0.00513 \times 5$\\
$\qquad \quad = 2.67$}
...
\answer{2cm}{difference = $(25-10) \times \hat{\beta}_{study} = 15 \times 0.0127933 = 0.19$\\
...
\begin{tabular}{ccc}
(A) & (B) & (C) \\
\includegraphics[width=2.7in]{./figures/regress_gpa_age_gender_study_miles_cprplot_study} &
\includegraphics[width=2.7in]{./figures/regress_gpa_age_gender_study_miles_rvfplot} &
\includegraphics[width=2.7in]{./figures/regress_gpa_age_gender_study_miles_qnorm_ehat} 
\end{tabular}
...
\answer{3cm}{$E(M) = \beta_0 + \beta_1 GPA + \beta_2 POP + \beta_3 DEPRESS$\\
...
(Stata code to calculate: \texttt{.display Ftail(3, 85, 14.83)}\\
...
\answer{3.5cm}{GPA: coefficient is negative; as GPA goes up, marijuana use score tends to go down\\
...
\answer{3.5cm}{$\hat{\beta}_{GPA} = -0.597$; A one point increase in GPA is associated with a 0.597 point decrease in marijuana use score, controlling for popularity score and depression score (p$<$0.001).}
...
\answer{2cm}{Might be underreporting of marijuana use; possibly underreporting of depression.}
...
\answer{2cm}{$H_0: \beta_1=\beta_2=0$ versus $H_a: \beta_1 \ne 0$ and/or $\beta_2 \ne 0$\\
test statistic: $F=15.95$; 
Under $H_0$, $F \sim F(p, n-p-1) = F(2, 12-2-1) = F(2,9)$\\
...
\answer{2cm}{The estimated mean weight of children who are 0 years old and 0 inches tall is 6.55 lbs.\\
...
\answer{2cm}{The estimated mean weight increases by 0.72 lbs for each 1 inch increase in height, controlling for age. The effect is significant (p=0.022).}

(d) What is the estimated change in weight for a 1 foot increase in height?

\answer{1.5cm}{1 foot = 12 inches; $12 \times 0.722 = 8.7$ lbs}

(e) Is the effect of age on weight significant after controlling for height? (Cite a p-value.)

\answer{1.5cm}{No (but just barely non-significant); p-value = 0.056}
...
\answer{2cm}{$\beta_0$: CI will include 0 (p-value $>$ 0.05)\\
...
\answer{1.5cm}{$R^2$ stays the same or go down (can't go up); Adjusted $R^2$ might go up or down, don't know for sure}
