...
\answer{1.5cm}{The estimated mean SBP for a person who is 0 years old is 98.7 mmHg.\\
Not meaningful; these are adults and thus one can't be 0 years old.}

(b) Interpret the slope estimate.

\answer{1.5cm}{The estimated mean change in SBP for a 1 year increase in age is 0.97 mmHg.\\
...
\answer{2cm}{$98.71 + .971 \times 30 = 127.8$ mmHg}
...
\answer{2cm}{$45-30=15$ year difference\\
...
\answer{2cm}{Yes, there is evidence of a significant association, the p-value for the slope (``age" term) is $<0.0005$ (or, can look at the overall F-test since there is only one $X$: p-value = $0.0001$)}

(f) What is the coefficient of determination for this model? Write a one sentence interpretation of this quantity.

\answer{2cm}{$R^2=0.4324$ \quad 43\% of the variability in SBP is explained by age.}
...
\answer{1.5cm}{$\widehat{MPG}=39.4 - 0.006 \times POUNDS$}

(b) Interpret the number 39.44 in the above output.

\answer{1.5cm}{The estimated mean MPG for a car that weighs 0 pounds is 39.4 miles per gallon.}
...
\answer{2cm}{For every 1 pound increase in the weight of a car, the estimated mean MPG decreases by 0.006 miles per gallon.}

(d) What is the estimated change in MPG for a 1 U.S. ton increase in car weight? (Note: 1 U.S. ton = 2000 pounds)

\answer{3cm}{$2000 \times \hat{\beta_1} = 2000 \times -0.00601 = -12.012$ MPG, i.e., 1 ton increase in weight corresponds to a 12 MPG decrease.}

(e) Test whether there is a significant effect of weight on mileage.

\answer{4cm}{Model: $E(MPG) = \beta_0 + \beta_1 \times MPG$\\
Hypotheses:\\
$H_0: \beta_1=0$\\
$H_1: \beta_1=1$\\
...
Stata code to find $P(t_{72} > | -11.6|)$: \texttt{display ttail(72, 11.6)}\\
Reject $H_0$\\
...
\answer{4cm}{Because we rejected $H_0: \beta_1=0$, we \underline{do not} expect 0 to be in the interval.\\
...
$t_{n-2}^* = t_{72}^* = 1.99$\\
Stata code to find critical value: \texttt{display invttail(72, 0.025)}\\
...
\answer{2cm}{$R^2 = \frac{MSS}{TSS} = \frac{1591.9902}{2443.45946} = 0.65$}

(h) Calculate the correlation between MPG and pounds.

\answer{2cm}{$r = sign(\hat{\beta}_1) \times \sqrt{R^2} = - \sqrt{0.65} = -0.81$}
...
\answer{2cm}{Slope will change, $R^2$ will not change}

(j) The p-value for the overall F-test is missing from the output. What is it?

\answer{1.5cm}{p-value from overall F-test = p-value from test of slope (when there is just one $X$)\\
...
\begin{tabular}{ccc}
(A) & (B) & (C) \\
\includegraphics[width=2.7in]{./figures/regress_mpg_pounds_qnorm_ehat} &
\includegraphics[width=2.7in]{./figures/regress_mpg_pounds_hist_ehat} &
\includegraphics[width=2.7in]{./figures/regress_mpg_pounds_rvfplot} 
\end{tabular}
...
\answer{1.5cm}{$R^2 = \frac{MSS}{TSS} = \frac{5749.11511}{55949.858} = 0.103$}
